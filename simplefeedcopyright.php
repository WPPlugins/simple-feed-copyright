...
function simplefeedcopyright($content) {
    $content = $content.'<p>Original article: <a href="' . get_permalink() . 
'" rel="bookmark" title="Permanent link to \''.get_the_title().'\'">' .get_the_title(). '</a><p>&copy;' . date("Y") . ' <a href="' . get_bloginfo('url') . '">' . get_bloginfo('name') . '</a>. All Rights Reserved.</p>';
    return $content;
}
